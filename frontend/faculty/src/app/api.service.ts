...
export class ApiService {
  ...
  private setHeaders(): Headers {
    ...
    if (JSON.parse(localStorage.getItem('facultyInfo')) != null) {
     if (JSON.parse(localStorage.getItem('facultyInfo')).token ) {
       headersConfig['token'] = JSON.parse(localStorage.getItem('facultyInfo')).token;
     }
    } else {
      headersConfig['token'] =
  ...
  put(path: string, body: any): Observable<any> {
    let url = environment.api_url+'/'+path;
    //body.token = JSON.parse(localStorage.getItem('facultyInfo')).token;
